...
class SandShapingArena(Arena):
    ...
    def __init__(
        ...
        has_legs=True,
        xml="models/assets/arenas/sand_shaping_arena.xml",
        sand_sim_config=None,
        sand_box_offset=(0.0, 0.0, 0.0),
        mujoco_arena_origin=(0.0, 0.0, 0.0)
    ):
        super().__init__(xml)

        self.mujoco_arena_origin = mujoco_arena_origin
        ...
        self.sand_box_walls_body = self.worldbody.find("./body[@name='sand_box_walls']")
        self.sand_box_body = self.worldbody.find("./body[@name='sand_box']")
        ...
        self.sand_box_asset = self.asset.find("./hfield[@name='sand']")

        self.sand_sim_config = sand_sim_config
        self.sand_box_offset = sand_box_offset
        self.sand_box_dims = self.sand_sim_config["sand_box_dims"]
        self.grid_size = self.sand_sim_config["grid_size"]
        if type(self.grid_size) == list:
            self.sand_box_nrow = self.grid_size[0]
            self.sand_box_ncol = self.grid_size[1]
        else:
            self.sand_box_nrow = self.grid_size
            self.sand_box_ncol = self.grid_size

        self.configure_location()
        self.configure_sand_box()
    
    def configure_sand_box(self):
        self.sand_box_asset.set(
            "nrow", str(self.sand_box_nrow)
        )
        self.sand_box_asset.set(
            "ncol", str(self.sand_box_ncol)
        )
        self.sand_box_asset.set(
            "size", array_to_string(self.sand_box_dims)
        )
    ...
    def configure_location(self):
        ...
        self.sand_box_walls_pos = - np.array(self.mujoco_arena_origin) - np.array(self.sand_box_offset) #np.array([0.55, 0.0, 0.72])
        self.sand_box_walls_body.set("pos", array_to_string(self.sand_box_walls_pos))

        
        self.sand_box_pos = self.sand_box_walls_pos + np.array([0.0, 0.0, self.sand_box_dims[3]]) #np.array([0.55, 0.0, 0.73])
        self.sand_box_body.set("pos", array_to_string(self.sand_box_pos))
