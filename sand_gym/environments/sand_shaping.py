...
class SandShaping(SingleArmEnv):
    """
    ...
    def __init__(
        ...
        self.required_tool_out_steps = 3
        self.steps_until_tool_exit = None
        self.tool_outside_ctr = 0
        self.tool_touched_sand = False
    ...
    def update_mujoco_heightfield(self):
        self.sim.model._model.hfield_data = self.sand_simulator.get_normed_heightfield(flat=True, correct_render=self.correct_render)
        self.sim.forward()
        self.sim.step()
        self.sync_mujoco_passive_viewer()

    def add_visual_element(self, vis_elem):
        self.add_visual_elements_to_mujoco_passive_viewer(vis_elem)
    ...
    def _setup_references(self):
        ...
        self.gripper_body_id = self.sim.model.body_name2id(self.gripper_body_name) # 11
        ...
        # tool_half_extends = self.sim.model.geom_aabb[self.sim.model.geom_name2id(self.gripper_geom_name)][3:]
        ...
        # robot_init_z_pos = self.table_top[2]+tool_height+self.initial_sand_height
    ...
    def reset_execution_step_counter(self):
        self.steps_until_tool_exit = None
        self.tool_outside_ctr = 0
        self.tool_touched_sand = False
    ...
    def _reset_internal(self):
        ...
        self.reset_execution_step_counter()
    ...
    def _post_action(self, action):
        ...
        tool_in_sand = self.sand_simulator.tool_in_sand_check()
        if tool_in_sand:
            self.tool_touched_sand = True
            self.tool_outside_ctr = 0
        else:
            if self.tool_touched_sand:
                self.tool_outside_ctr += 1
                if self.tool_outside_ctr == self.required_tool_out_steps:
                    self.steps_until_tool_exit = self.current_iteration - self.tool_outside_ctr
        
        if done:
            if self.steps_until_tool_exit is None:
                self.steps_until_tool_exit = self.current_iteration
            info["execution_steps"] = self.steps_until_tool_exit + 1
            if self.reward_utils is not None:
